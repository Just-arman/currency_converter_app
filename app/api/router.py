...
async def get_best_buy_rates(
    ...
    raw_result  = await CurrencyRatesDAO.find_best_buy_rates(session=session, usd=usd, eur=eur, count=count)
...
async def get_best_sell_rates(
    ...
    result = await CurrencyRatesDAO.find_best_sell_rates(session=session, usd=usd, eur=eur, count=count)
    if not result:
        raise HTTPException(status_code=404, detail=settings.ERROR_MESSAGES["not_found"])
    return result