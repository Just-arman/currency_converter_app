...
class CurrencyRatesDAO(BaseDAO):
    ...
    async def bulk_update_currency(cls, records: list[BaseModel], session: AsyncSession) -> int:
        ...
        try:
            ...
            cls._log_duplicate_banks(records_dict)
            parsed_records, parsed_bank_ens, parsed_bank_names = cls._prepare_parsed_records(records_dict)
            ...
            db_banks = await cls._fetch_db_banks(session)
            ...
            to_delete = {
                ...
                and db_banks[b].last_seen_at is not None
                and db_banks[b].last_seen_at < cutoff
            }

            counted_banks = 0  # количество банков без дублирований

            # 4. Обновляем/добавляем данные с суммированием их в переменную counted_banks
            # 4а. UPDATE (обновляем существующие; отмечаем время обнаружения и реактивируем,
            # если банк ранее был помечен неактивным, но снова появился в парсинге)
            counted_banks += await cls._apply_updates(session, parsed_records, to_update, datetime_now)

            # 4b. INSERT (добавляем новые)
            counted_banks += await cls._apply_inserts(session, parsed_records, to_add, datetime_now)

            # 5. ДЕАКТИВАЦИЯ + ОТЛОЖЕННОЕ УДАЛЕНИЕ: банки, не встречавшиеся
            ...
            # 5a. Банки, которые уже есть в бд, но которые не отобразятся при следующем парсинге — сразу помечаем неактивными
            await cls._apply_deactivation(session, to_deactivate)

            # 5b. Банки, неактивные дольше порога — удаляем из бд
            await cls._apply_deletion(session, to_delete)
    ...
    @classmethod
    def _log_duplicate_banks(cls, records_dict: list[dict]) -> None:
        """Логирует банки, дублирующиеся внутри одного прогона парсера."""
    ...
    @classmethod
    def _prepare_parsed_records(cls, records_dict: list[dict]) -> tuple[list[dict], set[str], set[str]]:
        """Отбрасывает записи без банков и дубликаты, возвращает чистый набор."""
        parsed_records = []
        parsed_bank_ens = set()
        ...
        return parsed_records, parsed_bank_ens, parsed_bank_names

    @classmethod
    async def _fetch_db_banks(cls, session: AsyncSession) -> dict:
        """Читает текущее состояние всех банков из БД."""
        ...
        return {row.bank_en: row for row in result.all()}
    ...
    @classmethod
    async def _apply_deactivation(cls, session: AsyncSession, to_deactivate: set[str]) -> None:
        """Помечает неактивными банки, пропавшие из текущего парсинга."""
        if to_deactivate:
            deactivate_stmt = (
                update(cls.model)
    ...
    @classmethod
    async def _apply_deletion(cls, session: AsyncSession, to_delete: set[str]) -> None:
        """Удаляет банки, неактивные дольше срока отсрочки."""
        if to_delete:
